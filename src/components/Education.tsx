...
export default function Education() {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-gradient-to-b from-gray-800 to-gray-900 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="max-w-4xl mx-auto relative mb-20">
          ...
          {education.map((edu, index) => (
            <div
              key={index}
              className={`relative pl-24 pb-12 transition-all duration-1000 ${
                isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'
              }`}
              style={{ transitionDelay: `${index * 300}ms` }}
            >
              <div className="absolute left-0 w-16 h-16 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center shadow-glow animate-pulse-slow">
                <GraduationCap className="w-8 h-8 text-white" />
              </div>

              <div className="glass-card p-8 rounded-2xl hover:scale-105 transition-transform duration-300">
                <h3 className="text-2xl font-bold text-white mb-2">{edu.degree}</h3>
                <h4 className="text-xl text-blue-400 mb-2">{edu.field}</h4>
              ...
              <div className="absolute left-8 top-16 w-16 h-0.5 bg-gradient-to-r from-blue-500 to-transparent"></div>
            </div>
          ))}
        </div>

        <div className={`max-w-5xl mx-auto pt-4 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}> 
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {coursework.map((c, i) => (
              <div key={i} className="glass-card p-6 rounded-2xl flex gap-4 items-start hover:scale-[1.02] transition-transform duration-300">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 flex items-center justify-center shadow-glow">
                  <Award className="w-7 h-7 text-white" />
                </div>
                <div className="flex-1">
                  <h4 className="text-lg font-semibold text-white mb-1">{c.title}</h4>
                  <p className="text-sm text-blue-300 mb-2">{c.provider}</p>
                  {c.link && (
                    <a
                      href={c.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-block text-sm font-medium text-purple-300 hover:text-purple-200 hover:underline"
                    >
                      View Credential ↗
                    </a>
                  )}
